...
function normaliseRow(raw: Record<string, string>): LocationRow {
  ...
  const handleFile = (file: File) => {
    ...
    if (ext === "csv") {
      Papa.parse(file, {
        ...
        skipEmptyLines: true,
        complete: (result) => {
          setRows((result.data as Record<string, string>[]).map(normaliseRow));
          setParsed(true);
        },
      ...
      reader.onload = (e) => {
        try {
          ...
          setError("Could not read Excel file. Please use the provided template.");
        }
      };
        ...
        {!parsed ? (
          <>
            <p style={{ fontSize: 13, color: "var(--ink-3)", marginBottom: 16, lineHeight: 1.6 }}>
              Upload a CSV or Excel file. Columns: <strong>label, address, city, state</strong>.
            </p>
            <button className="btn btn-secondary btn-full" style={{ marginBottom: 12, justifyContent: "center" }} onClick={downloadTemplate}>
            ...
            >
              {fileName ? (
                <div style={{ fontSize: 13, fontWeight: 600, color: "var(--blue)" }}>
                  📎 {fileName}<br />
                  <span style={{ fontSize: 11, color: "var(--ink-3)", fontWeight: 400 }}>Tap to change</span>
                </div>
              ) : (
                <>
                  <div style={{ fontSize: 28, marginBottom: 6 }}>📂</div>
                  <div style={{ fontSize: 13, fontWeight: 600 }}>Tap to select file</div>
                  <div style={{ fontSize: 11, color: "var(--ink-4)", marginTop: 4 }}>CSV or Excel (.csv, .xlsx)</div>
                </>
              )}
            </div>
          ...
          <>
            ...
            {validRows.length === 0
              ? <div className="alert alert-error" style={{ marginBottom: 12 }}>No valid rows found. Please check your file.</div>
